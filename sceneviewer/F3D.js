...
F3D Class for n64scene.js
shygoo 2018

*/

function mult(vec3, mat4)
{
    var res = [];
...
var F3D = {
    ...
    g_moveword: function(ucp)
    {
        var addr = ucp.cmd_b;

        var segment = (ucp.cmd_a >> 10) & 0x0F;
        var wordIdx = (ucp.cmd_a >>  0) & 0xFF;

        if(wordIdx == F3D.G_MW_SEGMENT)
        {
            ucp.segments[segment] = addr;
            console.log("assigning segment", segment, addr.toString(16));
        }
    },
    /*
    ...
    */
	...
	{	
		var nVertices = ((ucp.cmd_a >>> 20) & 0x0F) + 1;
		var vBufIdx = (ucp.cmd_a >>> 16) & 0x0F;
		var vAddr = ucp.cmd_b;
	...
	g_tri1: function(ucp)
	{
		var v0 = ((ucp.cmd_b >>> 17) & 0x7F) / 5 * 16;
		var v1 = ((ucp.cmd_b >>> 9) & 0x7F) / 5 * 16;
		var v2 = ((ucp.cmd_b >>> 1) & 0x7F) / 5 * 16;
        ...
        var vec0 = [v0_x, v0_y, v0_z];
        var vec1 = [v1_x, v1_y, v1_z];
        var vec2 = [v2_x, v2_y, v2_z];

        // transform via matrix
        mult(vec0, testmatrix);
        mult(vec1, testmatrix);
        mult(vec2, testmatrix);

        geo.vertices.push(new THREE.Vector3(vec0[0], vec0[1], vec0[2]));
        geo.vertices.push(new THREE.Vector3(vec1[0], vec1[1], vec1[2]));
        geo.vertices.push(new THREE.Vector3(vec2[0], vec2[1], vec2[2]));
	...
	g_settile: function(ucp)
	{
		...
		fmt = F3D.im_formats[fmt]
		siz = F3D.im_sizes[siz]
    ...
    */
    g_matrix: function(ucp)
    {
        var params = (ucp.cmd_a >> 16) & 0xFF;
        var addr = ucp.cmd_b;

        ucp.log('%cg_matrix: ' + params + ' ' + addr.toString(16), 'color: violet');

        var matrix = ucp.n64scene.getFixpointMatrix(addr);
        ...
        if(params & F3D.G_MTX_PROJECTION)
        {
        ...
        {
            testmatrix = matrix;
            console.log('modelview'); // implicit mul
        }

        if(params & F3D.G_MTX_PUSH)
        {
            console.log('push')
        }
        else
    ...
    g_popmatrix: function(ucp)
    {
        ucp.log('%cg_popmatrix:' , 'color: purple');
    }
}

F3D.im_formats = ['G_IM_FMT_RGBA', 'G_IM_FMT_YUV', 'G_IM_FMT_CI', 'G_IM_FMT_IA', 'G_IM_FMT_I'];
F3D.im_sizes = ['G_IM_SIZ_4b', 'G_IM_SIZ_8b', 'G_IM_SIZ_16b', 'G_IM_SIZ_32b'];
//F3D.movewd_indeces = ['G_MW_SEGMENT'];

F3D.G_MW_SEGMENT = 6;

F3D.G_MTX_PROJECTION  = 0x01;
F3D.G_MTX_MODELVIEW   = 0x00;

F3D.G_MTX_LOAD        = 0x02;
F3D.G_MTX_MUL         = 0x00; //

F3D.G_MTX_PUSH        = 0x04;
F3D.G_MTX_NOPUSH      = 0x00; //

F3D._class = {
    0xE7: function(){}, // pipesync
	0x04: F3D.g_vtx,
	//0x04: F3D.g_branch_z,
	0xBF: F3D.g_tri1,
	//0x06: F3D.g_tri2,
	//0x07: F3D.g_quad,
	//0xD9: F3D.g_geometrymode,
	0x06: F3D.g_dl,
    0xB8: F3D.g_enddl,
    0xBC: F3D.g_moveword,

    0x01: F3D.g_matrix,
    0xBD: F3D.g_popmatrix

	//0xE1: F3D.g_rdphalf_1,
	//0xE2: F3D.g_setothermode_l,
	//0xE3: F3D.g_setothermode_h,
	//0xE6: F3D.g_ucploadsync,
	//0xE7: F3D.g_ucppipesync,
	//0xF1: F3D.g_rdphalf_2,
	//0xF2: F3D.g_settilesize,
	//0xF3: F3D.g_loadblock,
	//0xF5: F3D.g_settile,
	//0xF9: F3D.g_setblendcolor,
	//0xFA: F3D.g_setprimcolor,
	//0xFB: F3D.g_setenvcolor,
	//0xFC: F3D.g_setcombine,
	//0xFD: F3D.g_settimg,
};