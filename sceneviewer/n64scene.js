...
function N64Scene(domSelector, config)
{
	...
	//this.segments = []; // segment bases for single block mode
	//for(var i = 0; i < 16; i++)
	//	this.segments[i] = 0x00000000;
	...
	this.singleBlockMode = config.singleBlockMode || false;
...
N64Scene.prototype = {
	...
	// single block mode
	setSegmentBase: function(segno, base)
	{
		if(!this.singleBlockMode)
		{
			throw Error('single block mode required')
		}

		this.ucp.segments[segno] = base;
	},
	segmentedToPhysical: function(segoffset)
	{
		if(!this.singleBlockMode)
		{
			throw Error('single block mode required')
		...
		var segno = (segoffset & 0x0F000000) >>> 24;
		var offset = segoffset & 0x00FFFFFF;
		return this.ucp.segments[segno] + offset;
	},
	// multi block mode
	loadSegment: function(segno, romStart, romEnd)
	{
		if(this.singleBlockMode)
		{
			throw Error('not supported by single block mode')
		}
	...
	getDVNum: function(segptr, dvFunction)
	{
		if(this.singleBlockMode)
		{
			return this.rom_dv[dvFunction](this.segmentedToPhysical(segptr));
		}
		...
		return this.segments_dv[segno][dvFunction](offset);
	},
	getU32: function(segptr)
	{
		return this.getDVNum(segptr, 'getUint32')
	},
	getU16: function(segptr)
	{
		return this.getDVNum(segptr, 'getUint16')
	},
	getU8: function(segptr)
	{
		return this.getDVNum(segptr, 'getUint8')
	},
	getS16: function(segptr) // vert
	{
		return this.getDVNum(segptr, 'getInt16')
	},
	getFloat: function(segptr)
	{
		return this.getDVNum(segptr, 'getFloat32')
	},
	...
	getFixpointMatrix: function(addr)
	{
        function fracbits2dec(bits, nbits)
        {
            var res = 0;
            var sh = nbits - 1;
		...
		for(var i = 0; i < 16; i++)
		{
			var col = (i / 4) | 0;
			var row = (i % 4);
			var intpart = this.getS16(addr + i*2);
			var fracpart = fracbits2dec(this.getU16(addr + i*2 + 32), 16);

			matrix[row][col] = intpart + fracpart;
		}

		if(matrix[0][3] != 0) alert(matrix[0][3])
		if(matrix[1][3] != 0) alert(matrix[1][3])
		if(matrix[2][3] != 0) alert(matrix[2][3])
...
N64Scene.UCP.prototype.reset = function()
{
	...
	this.stack = [];
	this.segments = [];
	for(var i = 0; i < 16; i++) this.segments[i] = 0;
